...
│   ├── Scripts/
│   │   ├── Core/                        # 핵심 매니저 (5개)
│   │   │   ├── ExperimentManager.cs         # 실험 전체 흐름 상태머신 (9 states)
...
│   │   │   ├── SessionSetupUI.cs            # 세션 설정 (참가자 ID, 카운터밸런싱)
...
│   │
│   ├── Data/
│   │   ├── CounterbalanceConfig.asset       # 카운터밸런싱 설정 (S1/S2)
│   │   ├── Routes/                          # 경로 데이터
│   │   ├── Missions/                        # 미션 데이터 (10개)
│   │   │   ├── RouteA_A1.asset, RouteA_B1.asset, RouteA_A2.asset, RouteA_B2.asset, RouteA_C1.asset
│   │   │   └── RouteB_A1.asset, RouteB_B1.asset, RouteB_A2.asset, RouteB_B2.asset, RouteB_C1.asset
│   │   ├── POIs/
│   │   │   ├── RouteA/                      # 9개 (room_b123~b132, room_b134, stairs_main)
│   │   │   └── RouteB/                      # 10개 (room_b101~b121, stairs_main_b)
│   │   ├── InfoCards/
│   │   │   ├── RouteA/                      # 5개 (a_card_01~05)
│   │   │   └── RouteB/                      # 5개 (b_card_01~05)
...
- `ParticipantSession`에서 참가자 ID, 순서 그룹(S1/S2) 로드
...
- Condition1/2 시작 시 `ConditionController.SetCondition()` 호출
...
- 경로 데이터에 연결된 미션 목록 순차 실행 (Route당 5개: A1→B1→A2→B2→C1)
- 미션 타입(A/B/C)별 분기 처리
- 검증 질문 정답 판정 및 로깅
...
var routeA_POIs = new POIData[] {
    ...
    new POIData {
        poiId = "room_b134", poiType = "project_room",
        displayName = "B134 프로젝트실", description = "그룹 작업용 프로젝트실",
        capacity = 0, equipment = new string[] { },
        mapPosition = new Vector2(0, 0)
    },
    new POIData {
...
var routeA_InfoCards = new InfoCardData[] {
    ...
    new InfoCardData {
        ...
        content = "좌측: B127 시청각실 | 우측: B125 대강의실 방면 | 직진: 서쪽복도(B129~B131)",
    ...
    new InfoCardData {
        ...
        title = "NW교차로 주변 시설",
        content = "좌측: B131 강의실 | 우측: B132 강의실, B134 프로젝트실 | 직진: 동쪽 방면",
        triggerWaypointId = "WP06", autoShow = true
    },
    new InfoCardData {
        cardId = "a_card_05", cardType = "comparison",
        title = "B132 vs B134 비교표",
...
var routeA_Comparison = new ComparisonData {
    ...
    itemNames = new[] { "B132 강의실", "B134 프로젝트실" },
...
7. 결과 표시 (Scored) → 2초 대기 → 다음 미션 또는 경로 종료
```
...
파일명: `{participantId}_{condition}_{route}_{timestamp}.csv`
예시: `P01_GlassOnly_RouteA_20260302_143025.csv`
...
| `ROUTE_START` | 경로 시작 | `{"route": "RouteA", "condition": "GlassOnly"}` |
...
- ScriptableObject 데이터 에셋 42개 (미션 10개, POI 21개, 정보 카드 10개, 설정 1개)
...
3. 현장 매핑 — 디지털관 B1, Route A/B 웨이포인트 앵커 생성
4. 파일럿 테스트 (5명) → 피드백 반영
5. 본 실험 수행 (24명)
